...
class ShopifyClientHelper extends Component
{
    ...
    protected $_shopifyRestApiVersion = '2020-01';

    protected $_shopifyGqlApiVersion = '2020-07';
    
    private $_isGraphql = false;

    private $urlFormat = '{shopify_domain}/admin/api/{version}/{endpoint}';

    public function __construct($shop_domain, $token, $api_key, $secret)
    {
        if ($shop_domain == 'ced_shopgdlf.myshopify.com') {
            $shop_domain = 'shopgdlf.myshopify.com';
        }
    ...
    public function curlHttpApiRequest($method, $raw_url, $query = '', $payload = '', $request_headers = array(), $attempt=1)
    {
        ...
        if(isset($this->last_response_headers['http_status_code']) && $this->last_response_headers['http_status_code']==429 && $attempt <= self::MAX_ATTEMPT) {
            ...
            $wait_time = 5;
            if(isset($this->last_response_headers['retry-after']) && is_numeric($this->last_response_headers['retry-after']) && $this->last_response_headers['retry-after'] < 5) {
                $wait_time = (int)$this->last_response_headers['retry-after'];
            }
            sleep($wait_time);
            return $this->curlHttpApiRequest($method, $raw_url, $query, $payload, $request_headers, $attempt++);
        }
        elseif(isset($this->last_response_headers['http_status_code']) && $this->last_response_headers['http_status_code']==500 && $attempt <= self::MAX_ATTEMPT) {
            sleep(5);
            return $this->curlHttpApiRequest($method, $raw_url, $query, $payload, $request_headers, $attempt++);
        }
        else {
    ...
    public function prepareApiVersionEndpoint($url)
    {
        if(strpos($url, 'oauth') === false)
        {
            ...
            if(count($explode) === 2)
            {
                ...
                // $url = strtr($this->urlFormat, ['{shopify_domain}'=>$shop, '{version}'=>$this->_shopifyApiVersion, '{endpoint}'=>$endpoint]);
                if($this->_isGraphql === true) {
                    $url = strtr($this->urlFormat, ['{shopify_domain}'=>$shop, '{version}'=>$this->_shopifyGqlApiVersion, '{endpoint}'=>$endpoint]);
                } else {
                    $url = strtr($this->urlFormat, ['{shopify_domain}'=>$shop, '{version}'=>$this->_shopifyRestApiVersion, '{endpoint}'=>$endpoint]);
                }
