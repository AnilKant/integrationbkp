...
namespace frontend\components\SyncProduct;
...
class WalmartProductSync extends Component
{
    ...
    public function execute()
    {
        ...
        if ($totalPages > $pageLimit) {
            ...
            QueryHelper::sqlRecords($query, [':cron_data' => json_encode($updateArray), ':cron_name' => 'sync_inventory_batch'], 'update');

            return true;
        } else {
            /*$query = "UPDATE `walmart_cron_schedule` SET `cron_data`= :cron_data WHERE `cron_name`= :cron_name";
            QueryHelper::sqlRecords($query, [':cron_data' => '', ':cron_name' => 'sync_inventory_batch'], 'update');*/
...
//        $data = QueryHelper::sqlRecords($query, [':merchant_id' => $this->merchant_id], 'column',$this->dbname);
    ...
    {
        $result = WalmartCronSchedule::find()->where(['cron_name' => 'sync_inventory_batch'])->one();
