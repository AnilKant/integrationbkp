...
class InventoryUpdate extends \yii\base\Component
{
    ...
    public function execute()
    {
        if ($this->dbconnection) {
            ...
            $merchantList = array_unique(array_merge($inventorySyncDisabledMerchants, $testShopList));
            $merchantIds = implode(",", $merchantList);
            ...
            $query = "SELECT `wp`.`merchant_id`, `wp`.`product_id`, IF(`wpv`.`option_id` IS NOT NULL, `wpv`.`option_id`, `jp`.`variant_id`) AS `variant_id`, IF(`wpv`.`option_id` IS NULL, 'simple', 'variant') AS `type`, IF(`wpv`.`option_id` IS NOT NULL, `jpv`.`option_sku`, `jp`.`sku`) AS `sku`, IF((`wpv`.`option_id` IS NOT NULL) AND (`wpv`.`fulfillment_lag_time`), `wpv`.`fulfillment_lag_time`, `wp`.`fulfillment_lag_time`) AS `fulfillment_lag_time`, IF(`wpv`.`option_id` IS NULL, IF(`wp`.`product_qty` IS NULL, `jp`.`qty`, `wp`.`product_qty`), IF(`wpv`.`option_qtys` IS NULL, `jpv`.`option_qty`, `wpv`.`option_qtys`)) AS `qty`, IF(`wpv`.`option_id` IS NOT NULL, `wpv`.`status`, `wp`.`status`) AS `status` FROM `walmart_extension_detail` `wed` INNER JOIN `walmart_product` `wp` ON `wed`.`merchant_id`=`wp`.`merchant_id` LEFT JOIN `walmart_product_variants` `wpv` ON `wp`.`product_id`=`wpv`.`product_id` INNER JOIN `jet_product` `jp` ON `jp`.`id`=`wp`.`product_id` LEFT JOIN `jet_product_variants` `jpv` ON `wpv`.`option_id`=`jpv`.`option_id` WHERE IF(`wpv`.`option_id` IS NOT NULL, `wpv`.`status`, `wp`.`status`) IN ('PUBLISHED', 'STAGE', 'Item Processing', 'UNPUBLISHED', 'READY_TO_PUBLISH') AND `wed`.`status` NOT IN ('License Expired' , 'Trial Expired') AND `wed`.`app_status` LIKE 'install' AND `wed`.`merchant_id` NOT IN (".$merchantIds.") LIMIT $offset, $limit";
