...
class NeweggProductSync extends Component
{
    ...
    public function execute()
    {
        ...
        for ($i = $current_page; $i < $pageLimit; $i++) {
            ...
            foreach ($products as $product) {
                ...
                productInfo::updateDetails($product,$this->syncFields,$this->merchant_id);
            }
        }
        ...
        if ($totalPages > $pageLimit) {
            ...
            $query = "UPDATE `newegg_cron_schedule` SET `cron_data`= :cron_data WHERE `cron_name`= :cron_name";
    ...
    {
        $merchantsList = ProductInventory::getInventorySyncDisabledMerchants();
    ...
    public function getProduct($page = null, $limit = 50)
    {
        if (!is_null($page) && !is_null($limit)) {
            $offset = ($page-1) * $limit;
            $query = "SELECT `product_id` FROM `newegg_product` WHERE `merchant_id`=:merchant_id LIMIT $offset,$limit";
        } else {
...
//        $data = QueryHelper::sqlRecords($query, [':merchant_id' => $this->merchant_id], 'column',$this->dbname);
    ...
    {
        $result = NeweggCronSchedule::find()->where(['cron_name' => 'sync_product_data'])->one();
    ...
    {
        $query = "SELECT `value` FROM  `newegg_config` WHERE `merchant_id`=:merchant_id AND `data`=:field_name";
