...
$urlRegistered = \yii\helpers\Url::toRoute(['newegg-shop-detail/view-merchant']);
...
<div class="newegg-shop-detail-index">
    ...
    <?php // echo $this->render('_search', ['model' => $searchModel]); 
        ...
        $loginUrl = $helper->getLoginasMerchantUrl('neweggmarketplace');
    ?>
    ...
    <?= GridView::widget([
        ...
        'rowOptions'=>function ($model){
            if ($model->app_status=='uninstall'){
                return ['class'=>'error'];
            }elseif (($model->purchase_status=='License Expired')||($model->purchase_status=='Trail Expired')||($model->purchase_status=='Trial Expired')){
                return ['class'=>'danger'];
            }elseif ($model->purchase_status=='Purchased'){
                return ['class'=>'success'];
            }elseif ($model->purchase_status=='Not Purchase'){
                return ['class'=>'review'];
            }elseif ($model->purchase_status=='Trial'){
                return ['class'=>'review'];
            }
...
/*.Yii::getAlias('@webneweggurl').'/site/managerlogin?ext='.$model['merchant_id'].'&&enter=true*/
            /*'id',*/
            [
                'class' => 'yii\grid\ActionColumn',
                'template' => '{view} {update} {link} {removeconfigfile}',
                'buttons' => [
                    'link' => function ($url,$model,$key) use ($loginUrl){
                        return '<a data-pjax="0" href="'.$loginUrl.$model->merchant_id.'" target="_blank">Login as</a>';
                        // return Html::a('Login As',$loginUrl.$data['merchant_id'],['title'=>'Login As']);
                    },
                    'view' => function ($url,$model)
                    {
                        return Html::a(
                            '<span class="glyphicon glyphicon-eye-open jet-data"> </span>',  ['/newegg-shop-detail/view','id'=>$model->id],['title' => 'View']
                    ...
                    {
                        return Html::a(
                            '<span class="glyphicon glyphicon-plus-sign"> </span>',['/newegg-shop-detail/update','id'=>$model->id],['title' => 'Update']
                        );
                    },
                    'removeconfigfile' => function ($url,$model)
                    {
                        return Html::a(
                            '<span class="glyphicon glyphicon-trash"> </span>',['/newegg-shop-detail/removeconfigfile','shop'=>$model->shop_url],['title' => 'Remove Config File']
                        );
                    },
            ...
            [
                ...
                'label' => 'Shop URL',
                'value' => function($data){
                    return "<a href='https://".$data->shop_url."' target='_blank'>".$data->shop_url."</a>";
                },
                'format'=>'raw',
            ],
            // 'shop_url:ntext',
            ...
            [
                ...
                'filter'=>array("Purchased"=>"Purchased","Not Purchased"=>"Not Purchase","License Expired"=>"License Expired","Trail Expired"=>"Trial Expired","Trail"=>"Trial"),
            ],
            'country_code',
            'uninstall_date',
            'install_status',
            [
                ...
                'filter'=>array("install"=>"Install","uninstall"=>"Uninstall"),
            ],
            [
