...
/**
 * ReverbShopDetailsSearch represents the model behind the search form about `backend\models\ReverbShopDetails`.
 */
class ReverbShopDetailsSearch extends ReverbShopDetails
{
    /**
     * @inheritdoc
     */
    ...
    public function rules()
    {
        return [
            ...
            [['shopurl','email','token', 'install_date', 'verified_email','verified_mobile','uninstall_dates','expire_date', 'ip_address', 'last_login', 'updated_at', 'created_at','status'], 'safe'],
        ];
    }
    ...
    {
        $query = ReverbShopDetails::find()->select([TableConstant::SHOP_DETAILS.'.*','merchant.shopurl','merchant.verified_email','merchant.email','merchant.seller_username','merchant.seller_password','merchant.verified_mobile',TableConstant::INSTALLATION.'.status','merchant.owner_name']);
        ...
        // $query->leftJoin('rakutenus_registration', '`rakutenus_registration`.`merchant_id` = `'.TableConstant::SHOP_DETAILS.'`.`merchant_id`');
        $dataProvider = new ActiveDataProvider([
            'query' => $query,
            'sort' =>[
                'attributes'=>[
                    'merchant_id','install_status','install_date','uninstall_dates','purchase_status','expire_date', 'ip_address', 'last_login', 'shopurl', 'email', 'config','fname','lname'
                ],
                'defaultOrder' => [
