...
use backend\models\NeweggShopDetailSearch;
...
/**
 * NeweggShopDetailController implements the CRUD actions for NeweggShopDetail model.
 */
class NeweggShopDetailController extends MainController
{
    ...
    /**
     * Lists all NeweggShopDetail models.
     * @return mixed
     */
    public function actionIndex()
    {
        ...
        $searchModel = new NeweggShopDetailSearch();
    ...
    {
        if (($model = NeweggShopDetail::findOne($id)) !== null) {
            return $model;
        } else {
    ...
    public function actionViewMerchant()
    {
        ...
        if($merchant_id)
        {
            ...
            {
                $merchantData=Data::sqlRecords("SELECT name, email, mobile, shipping_source, reference FROM newegg_registration WHERE merchant_id=".$merchant_id,"all");
                // print_r($merchantData);die("kk");
            }
            elseif($param=="Payment")
            {
                $merchantData=Data::sqlRecords("SELECT id,billing_on,activated_on,plan_type, status,recurring_data FROM `newegg_payment` WHERE merchant_id=".$merchant_id."","all");
    ...
    {
        $query = "SELECT * FROM `newegg_shop_detail` as `nsd` INNER JOIN `user` ON nsd.merchant_id = user.id";
...
// echo "<pre>";print_r($shopData);die;
    ...
    {
        $query = "SELECT `merchant_id`,`expire_date`,`purchase_status` FROM `newegg_shop_detail` WHERE `purchase_status` IN ('Trail' ,'Not Purchased','Purchased')";
            ...
            if ($model && !empty($model['expire_date'])) {
                ...
                if (time() > $expdate) {
                    if ($model['purchase_status'] == 'Purchased') {
                        $sql = "UPDATE `newegg_shop_detail` SET `purchase_status`='License Expired' WHERE `merchant_id`='" . $merchant_id . "'";
                        $result = Data::sqlRecords($sql, null, 'update');
                    } elseif ($model['purchase_status'] == 'Trail') {
                        $sql = "UPDATE `newegg_shop_detail` SET `purchase_status`='Trail Expired' WHERE `merchant_id`='" . $merchant_id . "'";
                        $result = Data::sqlRecords($sql, null, 'update');
                    } elseif ($model['purchase_status'] == 'Trail') {
                        $sql = "UPDATE `newegg_shop_detail` SET `purchase_status`='Trail Expired' WHERE `merchant_id`='" . $merchant_id . "'";
                        $result = Data::sqlRecords($sql, null, 'update');
    ...
    public function actionRemoveconfigfile(){
        $shopurl=$_GET['shop'];
        $base_path = \Yii::getAlias('@frontend').'/modules/neweggmarketplace/config/'.$shopurl;
        if (file_exists($base_path)) {
            unlink(\Yii::getAlias('@frontend').'/modules/neweggmarketplace/config/'.$shopurl.'/'.'config.php');
        }
        if(is_dir($base_path)){
            rmdir($base_path);
        }
    ...
    {
        $query = "SELECT * FROM `newegg_can_payment` as `np` INNER JOIN `newegg_can_shop_detail` as nsd ON nsd.merchant_id = np.merchant_id where np.`activated_on`>='2019-07-01 00:00:00'";
        $shopData = Data::sqlRecords($query,'all');
