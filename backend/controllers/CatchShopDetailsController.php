...
use Yii;
use backend\models\CatchShopDetails;
use backend\components\Data;
use backend\components\catchmp\TableConstant;
use backend\models\CatchShopDetailsSearch;
...
/**
 * CatchShopDetailsController implements the CRUD actions for CatchShopDetails model.
 */
class CatchShopDetailsController extends MainController
{
    /**
     * @inheritdoc
     */
    ...
    {
        $searchModel = new CatchShopDetailsSearch();
...
// echo "<pre>";print_r($dataProvider);die;
    ...
    public function actionUpdate($id)
    {
        ...
        $sql="SELECT `value` FROM `catchmp_config` WHERE `merchant_id`=:merchant_id AND `config_path`=:config_path";
        ...
        if ($model->load($post_data) && $model->save()) {
            Data::sqlRecords('UPDATE `merchant` SET `seller_username`="'.$post_data["CatchShopDetails"]["seller_username"].'",`seller_password`="'.$post_data["CatchShopDetails"]["seller_password"].'" WHERE `merchant_id`='.$model['merchant_id'],null,'update','admin');
            Data::sqlRecords('INSERT IGNORE INTO `catchmp_config`(`merchant_id`,`config_path`,`value`) VALUES ("'.$model['merchant_id'].'","barcode_exempted", "'.$post_data["CatchShopDetails"]["barcode_exempted"].'")ON DUPLICATE KEY UPDATE `value`=VALUES(`value`)',null,'update','admin');

            Data::removeconfigfile($model->shopurl,'catchmp','newapp');
            return $this->redirect(['view', 'id' => $model->id]);
        } else {
    ...
    /**
     * Deletes an existing CatchShopDetails model.
    ...
    {
        if (($model = CatchShopDetails::findOne($id)) !== null) {
            return $model;
        } else {
    ...
    public function actionValidateShop(){
        ...
        $sql="SELECT `expire_date`,`token`,`shopurl`,`purchase_status`,`esd`.`merchant_id` FROM `catchmp_shop_details` `esd` 
        ...
        foreach ($result as $key => $value){
            ...
            $sc=new ShopifyClientHelper($value['shopurl'],$value['token'],TableConstant::CATCH_APP_KEY,TableConstant::CATCH_APP_SECRET);
            ...
            if($value['expire_date']!='0000-00-00 00:00:00' && time()>strtotime($value['expire_date']) && !is_null($value['expire_date'])){
                ...
                if($value['purchase_status']!=2 && $value['purchase_status']!=4){
                    ...
                    if(!self::checkRecurringStatusOnShopify($sc, $merchant_id)){
                        ...
                        $query="UPDATE `catchmp_shop_details` SET `expire_date`='".$expire_date."',`purchase_status`=2  WHERE merchant_id=$merchant_id";
            ...
            if(isset($shopjson['errors'])){
                if($shopjson['errors']=='[API] Invalid API key or access token (unrecognized login or wrong password)'){
                    $query="UPDATE `catchmp_shop_details` SET `install_status`=0  WHERE merchant_id=$merchant_id";
                    Data::integrationSqlRecords($query,null,"update");
                    $uninstall_count++;
                }/*else{
    ...
    {
        $recurringCharges = Data::integrationSqlRecords('SELECT `id` FROM `catchmp_payment` WHERE merchant_id="'.$merchant_id.'" AND `plan_type`="Recurring Plan (Monthly)"','one','select');
